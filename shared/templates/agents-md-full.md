...
- Use `planning-config.json` to resolve the planning root. `planning-config.local.json` may hold local paths for externally targeted repositories and must remain uncommitted.
